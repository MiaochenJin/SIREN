...
def _get_isoscalar(isoscalar):
    ...
    if isoscalar:
        raise ValueError("Charm specific PDF4LHC splines is configured with nuclear targets!")
...
def load_processes(
    ...
    for process_type in process_types:
        for primaries, nunubar in [[neutrinos, "nu"], [antineutrinos, "nubar"]]:
            if not isoscalar:
                dxs_file = os.path.join(base_path, f"dsdxdy_{nunubar}-N-{process_type}-charm-EPPS21nlo_CT18Anlo_O16_central.fits")
                xs_file = os.path.join(base_path, f"sigma_{nunubar}-N-{process_type}-charm-EPPS21nlo_CT18Anlo_O16_central.fits")
